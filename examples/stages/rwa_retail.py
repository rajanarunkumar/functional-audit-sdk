...
Contract parameters (reporting_period, unbanded_rw) arrive as keyword arguments: the
contract default unless a WHAT_IF run overrides them through RunOptions.scenario_params."""
from pyspark.sql import functions as F
import functional_audit as fa


@fa.stage("rwa_retail.ead")
def build_ead(inputs, reporting_period: str):
...
def build_rwa(inputs, reporting_period: str, unbanded_rw: float):
    ...
    return (joined.withColumn("rw", F.coalesce(F.col("rw_band"), F.lit(unbanded_rw)))
