...
RETURNS TABLE (
  ...
  logic_hash STRING, run_kind STRING, run_purpose STRING, scenario_id STRING,
  ...
  run_as_principal STRING, started_at TIMESTAMP, ended_at TIMESTAMP,
...
RETURN
  SELECT r.run_id, c.contract_id, c.contract_version, c.calculation_id,
         ...
         try_cast(c.body:business_logic AS STRING) AS business_logic,
         r.logic_hash_executed, r.run_kind, r.run_purpose, r.scenario_id,
         r.supersedes_run_id, r.reporting_period,
         r.entity_type, r.entity_id, r.entity_run_id, r.compute_id,
         r.run_as_principal, r.started_at, r.ended_at, r.output_commit_version,
         (SELECT collect_list(struct(object_name, delta_version))
            FROM ${CATALOG}.${SCHEMA}.run_inputs i WHERE i.run_id = r.run_id) AS inputs_pinned,
         ...
         x.stage, x.status,
         (SELECT first(decision) FROM ${CATALOG}.${SCHEMA}.attestations a
           WHERE a.run_id = r.run_id ORDER BY a.created DESC) AS attestation_decision
...
SELECT r.run_id, r.contract_id, r.contract_version, c.calculation_id, c.requirement_id,
       ...
       r.logic_hash_executed, r.entity_type, r.entity_id, r.entity_run_id,
       r.run_as_principal, r.started_at, r.ended_at, r.status,
       x.stage AS reconciliation_stage, x.status AS reconciliation_status,
       (SELECT count(*) FROM ${CATALOG}.${SCHEMA}.run_inputs i WHERE i.run_id = r.run_id) AS inputs_declared,
